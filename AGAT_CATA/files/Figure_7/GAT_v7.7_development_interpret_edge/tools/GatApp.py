...
class GatApp(object):
    ...
    def get_energy(self, graph, fname='tmp'):
        with tf.device(self.device):
            # graph.to(self.device)
            energy = tf.reduce_sum(self.energy_model(graph, fname)).numpy()
        return energy

    def get_energies(self, graph, fname='tmp'):
        with tf.device(self.device):
            # graph.to(self.device)
            return self.energy_model(graph, fname).numpy()

    def get_forces(self, graph, fname='tmp'):
        with tf.device(self.device):
            # graph.to(self.device)
            return self.force_model(graph, fname).numpy()
