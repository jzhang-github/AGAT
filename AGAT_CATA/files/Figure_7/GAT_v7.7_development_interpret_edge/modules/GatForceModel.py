...
class GAT(tf_model):
    ...
    def call(self, graph, fname='tmp'): #, Training=None, moving_mean=None, moving_var=None):
        '''
        Description:
        ----------
        ...
        with graph.local_scope():
            ...
            for l in range(self.num_gat_layer):
                h = self.gat_layers[l](h, dist, graph,
                                       fname=f'{fname}_layer_{l}_force')                 # shape of h: (number of nodes, number of heads * num_out)
            ...
            if not 'tmp' in fname.split('_'):
                save_graphs(os.path.join('graphs', f'graph_{fname}_force_score.bin'), graph, {'layer': tf.constant(0)})
