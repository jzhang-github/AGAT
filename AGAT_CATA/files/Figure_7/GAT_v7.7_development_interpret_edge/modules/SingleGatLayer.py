...
class GATLayer(layers.Layer):
    ...
    def call(self, feat, dist, graph, fname='tmp'): # feat with shape of (number of nodes, number of features of each node). The graph can have no values of nodes.
        """
        Description:
        ----------
        ...
        if not 'tmp' in fname.split('_'):
            save_graphs(os.path.join('graphs', f'graph_{fname}.bin'), graph, {'layer': tf.constant(0)})
